<template>
    <section class="min-h-screen flex items-center justify-center relative overflow-hidden bg-gray-900">
      <div class="text-center z-10">
        <div class="mb-8 relative">
          <div class="w-48 h-48 mx-auto relative">
            <svg class="w-full h-full" viewBox="0 0 200 200" @mousemove="moveEyes">
              <!-- Face -->
              ...
              <g class="eyes">
                ...
                <circle :cx="70 + eyeX" :cy="80 + eyeY" r="10" fill="black" />
                <circle :cx="130 + eyeX" :cy="80 + eyeY" r="10" fill="black" />
              </g>
              
              <!-- Mouth -->
              <path d="M 70 130 Q 100 160 130 130" fill="none" stroke="black" stroke-width="5" />
            </svg>
          </div>
        </div>
  ...
  const moveEyes = (event) => {
    ...
    const maxMove = 5
    eyeX.value = ((event.clientX - avatarCenterX) / rect.width) * maxMove
    eyeY.value = ((event.clientY - avatarCenterY) / rect.height) * maxMove
  }
